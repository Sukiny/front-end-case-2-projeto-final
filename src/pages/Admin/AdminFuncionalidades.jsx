...
function AdminFuncionalidades() {
    ...
    async function handleSubmit(event) {
        ...
        if (!response.ok) {
            toast.error('Erro ao cadastrar funcionalidade')
            return
        }
        const funcionalidade = await response.json()
        toast.success('Funcionalidade cadastrada com sucesso')
    ...
    async function excluirFuncionalidade(id) {
        ...
        toast.error('Funcionalidade excluída com sucesso')
    ...
    async function salvarEdicao(botao, id) {
        ...
        if (!response.ok) {
            toast.error('Erro ao editar funcionalidade')
            return
        }
    ...
    return (<>
        ...
        <Container className='conteudo-margin'>
            <div id="login-card" className="card " style={{marginBottom:"200px"}}>
                <div className="card-body">
                    <h2 className="text-center">Adicionar Funcionalidade</h2>
                    <br />
                    <Form onSubmit={handleSubmit}>
                        <Form.Group controlId="title" >
                            <Form.Label>Título</Form.Label>
                            <Form.Control type="text" placeholder="Digite o título da funcionalidade" className="form-control form1" />
                        </Form.Group>
